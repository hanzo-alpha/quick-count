  ...
  <div class="dropup">
    ...
    <div class="dropdown-menu dropdown-menu-right">
      ...
      <a class="dropdown-item modal-loader" data-toggle="modal"
         data-target="#modal-edit" id="btnEditModal" data-url="{{ $edit['url'] }}" title="EDIT SUARA CALON" href="#">
        <i class="bx bx-edit-alt mr-1"></i> {{ __('Edit Suara Calon')
            }}</a>
      {{--        @endcan--}}
      {{--        @can($delete['role'])--}}
      <form id="form-delete" method="post" action="{{ $delete['url'] }}" style="display: inline">
        @method('DELETE')
        @csrf
        <button title="Hapus Data TPS" class="dropdown-item" type="button"
          ...
          {{ __('Hapus Suara Calon') }}
        </button>
        {{--        <button title="Hapus Data TPS" class="dropdown-item btn-delete" id="hapus" type="button">--}}
        {{--          <i class="bx bx-trash mr-1"></i>--}}
        {{--          {{ __('Hapus Suara Calon') }}--}}
        {{--        </button>--}}
      </form>
...
@else
  <div class="d-flex inline-flex align-items-md-center">
    ...
    <form method="post" action="{{ $delete['url'] }}" style="display: inline">
      @method('DELETE')
      @csrf
      <button title="Hapus Data TPS" class="btn btn-link p-0 btn-delete" id="hapus" type="button"
              onclick="confirm('{{ __('Apa anda yakin ingin menghapus Calon ini?') }}') ? this.parentElement.submit() : ''">
